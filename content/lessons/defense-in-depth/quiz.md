...
A. הצבת בקרות נוספות ועצמאיות מאחורי ה-DMZ — חומת אש פנימית ו-IDS/IPS — שבודקות מחדש את התעבורה בדרכה פנימה, כך שנפילת החומה החיצונית אינה פותחת נתיב לשרתי הליבה.

B. הגדרת כללי ניתוב (Routing) נוקשים יותר באותה חומת אש שנפרצה, כך שתמשיך לשמש מעגל הגנה בפני עצמה גם לאחר ההשתלטות.

C. סגירת כל הפורטים הלא-הכרחיים באותה חומת אש שנפרצה, כך שגם בידי התוקף היא תמשיך לחסום את התעבורה אל הפנים.

D. הצפנת כל התעבורה בין ה-DMZ לרשת הפנימית ב-TLS, כך שהתוקף שהשתלט על חומת האש לא יוכל להבין מה עובר דרכה.

E. הגדלת רמת הרישום (Logging) בחומת האש שנפרצה, כך שכל פעולה של התוקף תתועד ותאפשר לחסום אותו בזמן אמת.
...
A. אפשרות לפרוס שתי חומות אש מיצרנים וטכנולוגיות שונים, כך שחולשה או zero-day באחת מהן לא יאפשרו לתוקף לחצות גם את השנייה.

B. צמצום דרמטי של עלויות הרישוי והתחזוקה, שכן שתי חומות אש קטנות זולות מחומת אש אחת גדולה.

C. הקטנת כמות החומרה הנדרשת, שכן שתי חומות אש בטור חוסכות את הקונפיגורציה המורכבת של ממשקים מרובים.

D. ייתור הצורך בהצפנת התעבורה בין ה-DMZ לרשת הפנימית, מכיוון שיש חומת אש גם בחוץ וגם בפנים.
...
A. מפני שכל שלושת האזורים תלויים ברכיב אכיפה **אחד**; אם חומת האש נפרצת, לתוקף יש נתיב ישיר לכל האזורים — ואין שום בקרה עצמאית בין ה-DMZ לרשת הפנימית.
...
C. מפני שארכיטקטורת Three-Legged אינה מאפשרת להציב שרתי Web ב-DMZ, ולכן ממילא אין מה להגן עליו שם.

D. מפני שחומת אש אחת אינה יכולה לעמוד בנפח התעבורה של שלושה אזורים, וקריסתה תחת עומס היא רק שאלה של זמן.
...
A. חמש הבקרות אינן מגינות על השרת כלל מפני תוקף שיודע את הכתובת, מפני שכולן יושבות על נתיב אחד שאפשר לעקוף — זהו bypass path, והוא מבטל את כל הערימה.

B. ארבע מתוך חמש הבקרות עדיין מגינות; רק ה-CDN נעקף, שכן הוא הרכיב היחיד שתלוי בכתובת ה-IP.

C. ההגנה נשמרת, שכן ה-WAF וה-IDS בודקים את תוכן הבקשות ולא את הנתיב שדרכו הן הגיעו.

D. אין בעיה אבטחתית ממשית: דליפת כתובת IP היא בעיית פרטיות בלבד, שכן הכתובת אינה מקנה לתוקף הרשאה כלשהי.

E. ההגנה נשמרת בזכות עקרון ההגנה לעומק — חמש שכבות פרוסות, ולכן די בכך שאחת מהן תזהה את התקיפה.
...
B. שכבת הזיהוי כשלה; יש להחליף את ה-IDS במוצר מיצרן אחר, שכן המוצר הנוכחי לא הצליח לזהות את הנוזקה.

C. יש להעביר את ה-IDS למצב Behavior-based בלבד ולכבות את הזיהוי מבוסס החתימות, שכן החתימות לא סייעו במקרה הזה.

D. יש לחבר את ה-IDS לחומת האש בפרימטר, כדי שאם ה-IDS יחמיץ נוזקה — חומת האש תזהה אותה במעגל השני.

E. הכשל נובע מהיעדר שכבות: יש לרכוש מוצרי אבטחה נוספים לקצה הרשת, שכן ההגנה הקיימת דלילה מדי.
...
A. לחלק את התקציב בין רכש חומות אש ייעודיות (Network-based FW) בנקודות הכניסה, לבין תוכנת חומת אש (Host-based FW) שתיפרס על השרתים עצמם.

B. להשקיע את מלוא התקציב בחומת אש ייעודית אחת (Network-based FW) חזקה במיוחד, שתיתן מענה הן לשכבת הרשת והן לשכבת השרתים.

C. להשקיע את מלוא התקציב בפריסת מספר חומות אש ייעודיות (Network-based FW) בנקודות שונות ברשת, שייתנו מענה גם לשכבת השרתים.

D. להשקיע את מלוא התקציב בתוכנת חומת אש (Host-based FW) על כלל השרתים, שכן היא המקום שבו המידע נמצא בפועל.

E. להשקיע את מלוא התקציב במערכת IDS מרכזית, שתנטר גם את הרשת וגם את השרתים ותייתר את הצורך בחומות אש נוספות.
