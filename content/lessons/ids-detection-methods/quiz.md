...
A. המערכת מתריעה כל היום על True Positive, ולכן קשה לצוות להתמקד באירועים אמיתיים.

B. המערכת סובלת מ-Type II error, המתבטא בריבוי התראות שאינן מצביעות על איום אמיתי.

C. המערכת מתריעה כל היום על True Negative, ולכן קשה לצוות להתמקד באירועים אמיתיים.

D. המערכת מתריעה כל היום על False Positive — התראות שאין מאחוריהן תקיפה — ולכן קשה לצוות להתמקד באירועים אמיתיים.

E. המערכת מתריעה כל היום על False Negative, ולכן קשה לצוות להתמקד באירועים אמיתיים.

Correct: D

Explanation: התרחיש מתאר **התראה שנוצרה** (Positive) **ללא תקיפה מאחוריה** (False) — זו ההגדרה המדויקת של False Positive, המכונה גם Type I error. E שגויה: False Negative הוא ההפך הגמור — תקיפה שהתרחשה **ללא** שום התראה, ולכן הוא אינו יכול "להתריע כל היום"; FN הוא הטעות השקטה. A שגויה: True Positive היא התראה **נכונה** על תקיפה אמיתית — בדיוק מה שהצוות רוצה לקבל, ולא מקור התלונה. C שגויה: True Negative פירושו שאין תקיפה ואין התראה — מצב שאינו מייצר עומס כלל, שכן לא נוצרת בו שום התראה. B שגויה כפליים: Type II error הוא השם החלופי של False Negative ולא של False Positive, ולכן היא מצמידה את הסיווג הנכון של התופעה לתווית ההפוכה — בדיוק המלכודת שהשאלה בודקת.
...
A. משום שזיהוי מבוסס חתימות דורש משאבי עיבוד גבוהים, ולכן הוא מדלג על חלק מהחבילות בעומס.

B. משום שזיהוי מבוסס חתימות פועל רק על תעבורה מוצפנת, ותקיפות zero-day מגיעות בתעבורה גלויה.

C. משום שזיהוי מבוסס חתימות מייצר יותר מדי התראות שווא, והתקיפה נבלעת ביניהן.

D. משום שזיהוי מבוסס חתימות פועל ברמת ה-host בלבד, ותקיפות zero-day מגיעות מהרשת.

E. משום שחתימה נכתבת רק לאחר שהתקיפה כבר זוהתה ותועדה בעולם, ולכן תקיפה שטרם נראתה אינה יכולה להתאים לאף דפוס קיים.

Correct: E

Explanation: המגבלה היא **מבנית ולא מימושית**: השיטה משווה מול מסד נתונים של דפוסים ידועים, וכפי שמנסח המרצה — "Knowledge-based IDS only as good as database of attack signatures". תקיפה חדשה לחלוטין לא תועדה, ולכן אין לה חתימה, ולכן היא עוברת. זהו בדיוק המובן של "low in completeness". B שגויה: השיטה אינה קשורה להצפנה — אתגר התעבורה המוצפנת קיים, אך הוא חל על ה-NIDS בכללותו ולא מסביר את החמצת ה-zero-day. A שגויה והפוכה לעובדות: המרצה מציין דווקא "High performance with minimal processing power" ו-"Fast and explainable". D שגויה: זיהוי מבוסס חתימות מיושם הן ב-NIDS והן ב-HIDS; אין קשר בין השיטה למיקום הפריסה. C שגויה והפוכה: זיהוי מבוסס חתימות מייצר **מעט** התראות שווא ("often lower false positives") — ריבוי התראות השווא מאפיין דווקא את הזיהוי ההתנהגותי.
...
B. להרחיב את מודול הזיהוי ההתנהגותי כך שיסמן כל סטייה מה-baseline, קטנה ככל שתהיה.

C. להפעיל את מודול הזיהוי ההתנהגותי (Behavior-based) בלבד, ולכבות לחלוטין את מודול החתימות.
...
A. המערכת כוונה שמרנית מדי (too quiet), ולכן החמיצה את התקיפה — יש להרפות את הסף ולהגדיל את הרגישות.

B. הצפת ה-False Positive יצרה Alert Fatigue, וזו הפכה התראת True Positive אמיתית להחמצה תפעולית — כלומר עודף התראות שווא הוא שייצר את הכשל.

C. המערכת סבלה מ-False Negative מתמשך לאורך שלושת החודשים, ולכן לא זיהתה את הנוזקה בזמן.

D. מדובר ב-True Negative: מאחר שההתראות בחודשים הראשונים היו שגויות, המערכת סיווגה נכון את הפעילות הרפואית כלגיטימית.

E. מדובר בכשל של מודול החתימות, שלא קיבל עדכון מהספק ולכן לא זיהה את הנוזקה החדשה.

Correct: B

Explanation: השרשרת היא בדיוק זו שמנסח המרצה: "Too sensitive → alert fatigue → ignored alerts". ההתראות הרבות היו False Positive (התראה ללא תקיפה); הן שחקו את הצוות; ובנקודת האמת ההתראה האמיתית — טכנית **True Positive** — נבלעה ולא טופלה. התוצאה בעולם זהה ל-False Negative, אך המקור שלה תפעולי ולא טכני. C שגויה: המערכת **כן** הפיקה התראה על הנוזקה, ולכן זו אינה False Negative טכנית — הכשל היה בתגובה, לא בזיהוי. E שגויה: התרחיש קובע במפורש שהמערכת הפיקה התראה, כלומר הזיהוי עבד; בנוסף מודול הזיהוי ההתנהגותי אינו תלוי בעדכוני חתימות. A שגויה והפוכה: המערכת הייתה **רגישה מדי** ולא שקטה מדי — הגדלת הרגישות רק תחריף את ההצפה ואת ה-Alert Fatigue. D שגויה: True Negative פירושו שלא נוצרה שום התראה מלכתחילה; כאן נוצרו מאות התראות, וכל אחת מהן שהתבררה כלגיטימית היא False Positive בהגדרה.
...
A. מעבר ל-IPS מייתר את הצורך בכיוונון (Tuning), שכן החסימה האוטומטית מונעת ממילא את הנזק מכל התראה שגויה.

B. דרישות רגולטוריות המחייבות שימוש בחתימות מבוססות יצרן בלבד, ולכן אין לאפשר חסימה על בסיס זיהוי התנהגותי.

C. החשש מפגיעה בפעילות רפואית קריטית עקב חסימת שווא (False Positive), אל מול הרווח שביכולת לבלום תקיפות בזמן אמת — ולכן מקובל להתחיל ב-detect-only ולהחיל חסימה רק על חוקים בעלי ודאות גבוהה.

D. מאחר שה-IPS יושב inline, הוא מקטין את שיעור ה-False Positive לעומת IDS, ולכן המעבר בטוח מטבעו.

E. שימוש גורף במערכות מבוססות התנהגות (Behavior-based) בלבד, ללא הפעלת מנגנון חתימות, יבטיח כיסוי מלא של האיומים.

Correct: C

Explanation: זהו לב ה-operational reality של IPS אצל המרצה: "False positives can impact business"; "Blocking errors can cause outages"; "Often run in detect-only first"; "IPS is powerful — treat it like a production change". ב-IDS התראת שווא עולה שעת אנליסט; ב-IPS אותה טעות בדיוק הופכת ל**חסימה** של תעבורה לגיטימית — ובבית חולים משמעותה עצירת פעילות רפואית. B שגויה: אין דרישה כזו, וכיבוי הזיהוי ההתנהגותי היה מבטל את היכולת היחידה לזהות zero-day. A שגויה והפוכה: המעבר ל-IPS **מייקר** את מחיר הטעות ולכן הופך את הכיוונון לקריטי יותר, לא מיותר. E שגויה: הפעלת זיהוי התנהגותי בלבד היא בדיוק התצורה בעלת ה-low accuracy — היא תגדיל דרמטית את חסימות השווא, וזהו התרחיש המסוכן ביותר בבית חולים. D שגויה: המיקום ה-inline אינו משנה דבר בשיעור ה-FP — הוא רק משנה את **התוצאה** של FP מהתראה לחסימה, ולכן מגדיל את הסיכון ולא מקטין אותו.
...
A. הוא מפחית False Negative במחיר של הגדלת False Positive — כיסוי רחב יותר תמורת רעש רב יותר.

B. הוא אינו משנה את יחסי הטעויות, אלא רק את עומס העיבוד על מנוע הניתוח.

C. הוא מפחית False Positive מבלי לפגוע בכיסוי, שכן חוקים מהודקים אינם משפיעים על היכולת לזהות תקיפות אמיתיות.

D. הוא מפחית False Positive במחיר של הגדלת False Negative — פחות התראות שווא, אך יותר תקיפות שיוחמצו.

E. הוא מפחית גם False Positive וגם False Negative, שכן חוקים מהודקים הם חוקים מדויקים יותר.

Correct: D

Explanation: המרצה מצייר את ה-Tuning כ**ספקטרום** עם סיכון בכל קצה: "Risk if too quiet: Missed detections (false negatives)" מול "Risk if too noisy: Alert fatigue (false positives)". זו ידית אחת — כל תזוזה לכיוון השקט מפחיתה FP ומגדילה FN. E שגויה: זו בדיוק האשליה שהמטריצה מפריכה; אין נקודה על הספקטרום שבה שני סוגי הטעות יורדים יחד, ולכן המרצה מנסח את המטרה כ-"increase precision **without killing** recall" — כלומר כאילוץ ולא כניצחון כפול. A שגויה: היא מתארת את התזוזה ל**כיוון ההפוך** (הרפיית הסף), לא את הידוק החוקים שהשאלה מתארת. B שגויה: עומס העיבוד עשוי אכן לרדת, אך זו תופעת לוואי — ההשפעה המהותית היא על תמהיל הטעויות. C שגויה: "מבלי לפגוע בכיסוי" היא בדיוק ההבטחה הבלתי אפשרית; הידוק גורף של סף ההתראה **חייב** להקטין כיסוי, ולכן המרצה מחייב "validate with testing & incidents" אחרי כל כיוונון.
